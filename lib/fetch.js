...
import * as logInfo from './functions/getLogInformation';
                ...
                .end(async (err, result) => {
                    if (err) {
                        ...
                        console.log(logInfo.memoryUsage()
                            + ' '
                            + logInfo.concurrencyCount(concurrencyCount)
                            + ' '
                            + logInfo.errorMessage('[抓取失败]')
                            + ' '
                            + logInfo.normalYMessage('(' + id.toString() + ')')
                            + ' '
                            ...
                            await saveData({
                                ...
                                .catch(err => {
                                    ...
                                    console.log(logInfo.memoryUsage()
                                        ...
                                        + ' '
                                        + logInfo.errorMessage('[插入错误]')
                                        + ' '
                                        + logInfo.normalYMessage('(' + id.toString() + ' ' + sudokuType + ')')
                                        + ' '
                                        + logInfo.errorMessage(err.errors[0].message)
                                    );
                                });
